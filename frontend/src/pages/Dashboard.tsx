      ...
      <main style={{ marginLeft: '224px', flex: 1, padding: '2rem', overflowY: 'auto', position: 'relative', zIndex: 1, minWidth: 0 }}>
        ...
        <div style={{ ...card, padding: '1.5rem' }}>
          ...
          {loading ? (
            ...
            <div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 180px 130px 120px', padding: '0.5rem 1rem', marginBottom: '0.25rem' }}>
              ...
              {recent.map((c, i) => {
                ...
                return (
                  <div key={c.case_id}
                    style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 180px 130px 120px', padding: '0.875rem 1rem', borderRadius: '10px', alignItems: 'center', cursor: 'pointer', transition: 'background 0.15s', background: i % 2 === 0 ? 'rgba(255,255,255,0.02)' : 'transparent' }}
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                      ...
                      {c.is_overridden && <span style={{ color: '#F59E0B', fontSize: '0.65rem', fontWeight: 600 }}>✎ Overridden</span>}
                    </div>
                    <span style={{ color: '#94A3B8', fontSize: '0.8rem' }}>{formatDate(c.created_at)}</span>
                    <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', justifyContent: 'flex-end' }}
                      onClick={e => e.stopPropagation()}>
                      <button onClick={() => navigate(`/history/${c.case_id}`)} style={{ background: 'none', border: 'none', color: '#6366F1', fontSize: '0.8rem', cursor: 'pointer', fontFamily: 'inherit', padding: 0 }}>View →</button>
